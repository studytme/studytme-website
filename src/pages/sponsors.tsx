...
import sponsorsLocaleContent from "../../locales/sponsor";
...
const Sponsors: NextPage<{ locale: string; defaultLocale: string }> = (props: {
  locale: string;
  defaultLocale: string;
}) => {
  const localeContent = useMemo(
    () => sponsorsLocaleContent[props.locale || props.defaultLocale],
    [props.locale]
  );
    ...
    <div className="flex flex-col">
      ...
      <section className="flex flex-col-reverse items-center justify-between w-full sm:flex-row sm:h-headerless-screen">
        <div className="flex flex-col items-center my-10 sm:items-start">
          ...
          <p className="text-center lg:text-2xl sm:text-left">{localeContent.title}</p>
          <Socials />

          <OutlinedButton
            icon={<FaEnvelope />}
            text={localeContent.contact_button}
      ...
      <SponsorSection header="Audience">
        <div className="flex flex-col w-full gap-6 mb-10 sm:flex-row justify-evenly">
          <AudienceCard
            title={localeContent.total_followers.title}
            icon={<BsPeopleFill className="w-5 h-5 ml-2" />}
            main="82k+"
          />
          <AudienceCard
            title={localeContent.main_platform.title}
            icon={<FaTwitch className="w-5 h-5 ml-2" />}
            main="Twitch"
            additional={
              <p className="italic text-slate-400">{localeContent.main_platform.description}</p>
            }
          />
          <AudienceCard
            title={localeContent.average_ccv.title}
            icon={<FaTwitch className="w-5 h-5 ml-2" />}
            main="551"
            additional={
              <p className="italic text-slate-400">{localeContent.average_ccv.description}</p>
            }
          />
        </div>
      </SponsorSection>
